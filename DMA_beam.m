...
theta_target = 90*pi/180;             % target
alpha_1_1 = cos(210*pi/180);          % 零点方向
B = zeros(length(theta),length(f));
for i = 1:length(f)
    B(:,i) = 1j/(omega(i)*tao0*(alpha_1_1-1))*(1-exp(-1*1j*omega(i)*tao0*(cos(theta)-alpha_1_1)));
%     B(:,i) = (1-exp(-1*1j*omega(i)*tao0*(cos(theta)-cos(alpha))));
    H(:,i) = 1j/(omega(i)*tao0*(alpha_1_1-cos(theta_target)))*[1;
                                           -exp(1j*omega(i)*tao0*alpha_1_1)];
    H(:,i) = 1/(1-exp(1j*omega(i)*tao0*(alpha_1_1-cos(theta_target))))*[1;
                                           -exp(1j*omega(i)*tao0*alpha_1_1)];
    B(:,i) = 1/(1-exp(1j*omega(i)*tao0*(alpha_1_1-cos(theta_target))))*...
                                  (1-exp(-1*1j*omega(i)*tao0*(cos(theta)-alpha_1_1)));
end
...
figure,polarplot(linspace(0,2*pi,360),real(B(:,1000)));
figure, mesh(real(B(:,1:8000))),title('beampattren');
